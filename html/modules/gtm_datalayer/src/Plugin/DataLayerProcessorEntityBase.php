...
class DataLayerProcessorEntityBase extends DataLayerProcessorBase {
   ...
   *   The module handler.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, Request $current_request, PathMatcherInterface $path_matcher, LanguageManagerInterface $language_manager, AccountInterface $current_user, EntityTypeManagerInterface $entity_type_manager, RouteMatchInterface $current_route_match, DateFormatterInterface $date_formatter, Token $token, ModuleHandlerInterface $module_handler) {
  ...
  protected function getTags() {
    if ($this->moduleHandler->moduleExists('gtm_datalayer_override') && !$this->isRequestException() && $this->getEntity() instanceof EntityInterface) {
      $overrides = $this->entityTypeManager
        ->getStorage('gtm_datalayer_override')
        ->loadByMultipleRelatedEntity($this->getEntity());

      if (!empty($overrides)) {
        /** @var \Drupal\gtm_datalayer_override\Entity\DataLayerOverrideInterface $override */
        $override = reset($overrides);

        $this->dataLayerTags = array_replace_recursive($this->dataLayerTags, $override->getTags(FALSE));
      }
    }
