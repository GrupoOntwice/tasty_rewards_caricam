...
class DataLayerController extends ControllerBase implements DataLayerControllerInterface {
   ...
   */
  public function __construct(ModuleHandlerInterface $module_handler, ConfigFactoryInterface $configFactory, EntityStorageInterface $storage, ContextHandlerInterface $context_handler, ContextRepositoryInterface $context_repository, RendererInterface $renderer, RequestStack $request_stack) {
  ...
  public function isEnabled() {
    // Allow to be enabled/disabled per request.
    $enable = $this->requestStack->getCurrentRequest()->query->get('gtm_datalayer');

    return ($enable === NULL) ? $this->configuration->get('status') : (bool) $enable;
  }
  ...
  public function buildGtmScripts(array &$attachments) {
    // Add a new cache contexts for this module feature.
    $attachments['#cache']['contexts'][] = 'url.query_args:gtm_datalayer';

    // If is enabled and configured.
    if ($this->isEnabled() && $this->isConfigured()) {
      // Build Google Tag Manager dataLayer (script) code with the rendered
      // tags.
      if ($this->configuration->get('single_mode') == TRUE) {
        $data_layer = Json::encode($this->renderTags());
      }
      else {
        $data_layer = [];
        foreach ($this->renderTags() as $object) {
          $data_layer[] = Json::encode($object);
        }
        $data_layer = implode(', ', $data_layer);
      }
      $attachments['#attached']['html_head'][] = [
        [
          '#type' => 'html_tag',
          '#tag' => 'script',
          '#value' => "var dataLayer = [" . $data_layer . "];",
  ...
  /**
   * Allow other modules to alter the tags.
   *
   * @param array $tags
   *   The rendered dataLayer tags to alter.
   */
  protected function alterTags(array &$tags = []) {
    $this->moduleHandler->alter('gtm_datalayer_tags', $tags, $this);
  }
